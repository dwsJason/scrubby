...
int main(int, char**)
{
    ...
    SDL_Window* window = SDL_CreateWindow("Scrubby", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, sdl_window_flags);
...
//	PhotoShop_SetupImGuiStyle();
	...
	//io.Fonts->AddFontFromFileTTF("./data/Inconsolata.otf", 18.0f);
	//io.Fonts->AddFontFromFileTTF("./data/Inconsolata-Regular.ttf", 18.0f);
    ...
    while (!bAppDone)
    {
		...
		if (show_targets_window)
		{
			...
			TargetManager::GetInstance()->Render();
